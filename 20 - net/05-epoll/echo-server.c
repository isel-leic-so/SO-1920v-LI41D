...
int main(int argc, char *argv[]) {
	...
	struct epoll_event ev, clients[MAX_CLIENTS];
	...
	for (;;) {
		...
		int conn_fd;
		int nready = epoll_wait(events_fd, clients, nclients, INFTIM);
		printf("there are %d ready descriptors\n", nready);

		for (int i = 0; i < nready; ++i) {
			struct epoll_event * pev = clients + i;
			...
			if (pev->data.fd == 0 && pev->events & EPOLLIN) {
				...
				if (len > 0) {
					int wlen = write(conn_fd, buf, strlen(buf));
					if (wlen < 0) { 
